...
input_file='/users/jywang/MeerKAT/calibration2021/level1/MeerKLASS2021/mask/'
...
d3 = pickle.load(open(input_file+fname+'_'+ant+'_mask'))
print 'mask loaded'
mask_inter=d3['mask']
...
print Tnd_std
#Tnd_std,Tnd_ref,noise,Tnd_spl= km.call_Tnd(data, ant, pol,freqs,ch_plot,1) #single channel
...
for ch_plot in range(550,1051):
    ...
    if check_ratio>0.6:
        try:
            ...
            #select raw vis for track after scan
            ...
            if ch_plot==800:
                ...
                plt.ylim(-0.19,0.19)
                plt.title('$T_{res}$ of the fitted curve above')
                plt.savefig(output_file+'F_caliB_'+str(fname)+'_'+str(recv)+'_ch'+str(ch_plot)+'.png', bbox_inches='tight')
                #plt.show()
...
plt.xlabel('channel')
plt.ylabel('Tnd (K)')
plt.title(fname+', '+recv+' ('+target+')')
plt.savefig(output_file+'Tnd_'+str(fname)+'_'+str(recv)+'.png', bbox_inches='tight')
#plt.show()
